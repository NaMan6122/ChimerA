...
- Transports: browser (10–20s/turn) or browserless qwen web-API replay (~4s p50,
  18 MB RAM, no Chromium). Say both on every page with a CTA.
- Auth: `Authorization: Bearer`, `x-api-key`, `anthropic-api-key`.
- Endpoints: `POST /v1/chat/completions` (JSON + SSE streaming, tool-call deltas),
  `GET /v1/models`, `GET /v1/health/providers`, `GET /v1/usage`, `GET /metrics`.
  Planned: `/v1/responses`, `/v1/messages` (Anthropic shape), MCP stdio mode.
- Pricing: Hobby $0 self-host · Subscriber API $9–12/mo/account (browserless, 10k req) ·
  Browser Solo $19/mo/browser (5k req) · Team $79/mo (auto transport + fallback,
  50k req) · Scale custom. Per account/browser, not per token.
- Legal footer on every page: "Automating consumer AI subscriptions may violate vendor
  Terms of Service. Use your own accounts. Not affiliated with OpenAI, Anthropic, or others."

## Sitemap (5 pages)

1. `/` **Home** — hero (tagline + `docker compose up` snippet + "curl returns an answer"
   demo GIF placeholder `assets/demo.gif`), numbers strip (18MB binary / 5 providers /
   ~4s browserless vs ~14s browser, with honesty note), "Two ways to use it" cards
   (AS THE MODEL via `base_url` swap → code sample; AS A TOOL via MCP → Claude Code
   config snippet, mark "coming soon" if unshipped), provider logo row (text names
   only, no vendor logos), self-host-vs-managed table, pricing teaser (4 cards +
   Scale), FAQ preview (4), footer (GitHub, docs, disclaimer).
2. `/docs` **Docs** — quickstart (binary + Docker paths; browserless session export
   for qwen), configuration table (mirror `.env.example`: PROVIDER, TRANSPORT,
   AUTH_DIR, API_TOKEN(S), QUOTA_MONTHLY_REQUESTS, METER_DB, LOG_FORMAT…),
   API reference (each endpoint: method, auth, example request/response curl + Python),
   providers matrix (transport per provider), observability (metrics list + dashboard
   screenshot placeholder), troubleshooting (re-login via VNC, `POST /v1/refresh`,
   webapi session re-export, transport fallback metric).
3. `/pricing` **Pricing** — full tier table (feature matrix: substrate, providers,
   requests/mo, logs retention, dashboard, selector-fix SLA, support), overage note
   (per request on webapi, browser-hour on dom), FAQ (why per account/browser not
   per token? what counts as a request? what happens over quota? → 429
   `quota_exceeded` before provider work), CTA: deploy-template buttons.
...
- No performance claims beyond Facts; always pair speed claims with the
  "~4s browserless / ~14s browser" note.
