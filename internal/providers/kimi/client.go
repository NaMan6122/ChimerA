...
// Client implements the Provider interface for Kimi.
type Client struct {
	*providers.Base
}

// NewClient creates a new Kimi provider.
func NewClient(page *rod.Page, cfg *config.Config) *Client {
	return &Client{
		Base: providers.NewBase("kimi", "chimera-kimi", page, cfg),
	}
}

// Init prepares the Kimi provider.
func (c *Client) Init(page *rod.Page, cfg *config.Config) error {
	c.Page = page
	c.Cfg = cfg

	c.Log.Infof("Initializing Kimi provider (url=%s)", cfg.KimiURL)

	err := rod.Try(func() {
		c.Page.Timeout(30 * time.Second).MustNavigate(cfg.KimiURL)
	})
	if err != nil {
		return fmt.Errorf("navigating to Kimi: %w", err)
	}
	...
	if !loggedIn {
		return fmt.Errorf("not logged into Kimi — please log in via the browser window")
	}

	c.Log.Info("Kimi provider initialized successfully")
	return nil
}
...
// NewChat starts a new Kimi conversation.
func (c *Client) NewChat() error {
	c.Log.Info("Starting new Kimi conversation")
	...
	err := rod.Try(func() {
		c.Page.Timeout(15 * time.Second).MustNavigate(c.Cfg.KimiURL)
	})
	if err != nil {
...
// IsLoggedIn checks if the user is logged into Kimi.
