...
// Client implements the Provider interface for DeepSeek.
type Client struct {
	*providers.Base
}

// NewClient creates a new DeepSeek provider.
func NewClient(page *rod.Page, cfg *config.Config) *Client {
	return &Client{
		Base: providers.NewBase("deepseek", "chimera-deepseek", page, cfg),
	}
}

// Init prepares the DeepSeek provider.
func (c *Client) Init(page *rod.Page, cfg *config.Config) error {
	c.Page = page
	c.Cfg = cfg

	c.Log.Infof("Initializing DeepSeek provider (url=%s)", cfg.DeepSeekURL)

	err := rod.Try(func() {
		c.Page.Timeout(30 * time.Second).MustNavigate(cfg.DeepSeekURL)
	})
	if err != nil {
		return fmt.Errorf("navigating to DeepSeek: %w", err)
	}
	...
	if !loggedIn {
		return fmt.Errorf("not logged into DeepSeek — please log in via the browser window")
	}

	c.Log.Info("DeepSeek provider initialized successfully")
	return nil
}
...
func (c *Client) SendMessage(text string, threadID string) (*models.ProviderResponse, error) {
	...
	if err := c.WaitForResponse(StopButton); err != nil {
		...
		if err := c.waitForTextStability(); err != nil {
			c.Log.Warnf("Text stability failed: %v", err)
		}
	}
...
// NewChat starts a new DeepSeek conversation.
func (c *Client) NewChat() error {
	c.Log.Info("Starting new DeepSeek conversation")
	...
	err := rod.Try(func() {
		c.Page.Timeout(15 * time.Second).MustNavigate(c.Cfg.DeepSeekURL)
	})
	if err != nil {
...
// IsLoggedIn checks if the user is logged into DeepSeek.
