...
func TestGenerateCookiesShape(t *testing.T) {
	itna, itna2 := generateCookies(generateFingerprint())
	for name, v := range map[string]string{"ssxmod_itna": itna, "ssxmod_itna2": itna2} {
		if len(v) == 0 || v[0] != '1' || v[1] != '-' {
			t.Fatalf("%s: unexpected shape %q", name, v)
		}
	}
}

func TestGenerateBXUA(t *testing.T) {
	got := generateBXUA(generateFingerprint())
	if len(got) < 10 || got[:4] != "231!" {
		t.Fatalf("bx-ua shape: %q", got)
	}
}
